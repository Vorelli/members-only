...
exports.index = function (req, res, next) {
  const getMessages= {
    messages: function(cb) { Message.find().populate({path: 'user', select: 'firstName lastName'}).exec(cb); }
  }
    ...
    if(err) next(err);
      ...
      if(!res.locals.currentUser) 
        ...
        for (var i = results.messages.length - 1; i >= 0; i--) {
          results.messages[i].user = results.messages[i].user.firstName + ' ' + results.messages[i].user.lastName;
        }
      }
